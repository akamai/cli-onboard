...
Batch-create Akamai delivery configurations from CDN migration artifacts. Designed specifically for customers converting from competing CDNs (Cloudflare, CloudFront, Imperva, Fastly). Consumes the CSV and ruletree JSON files generated by CDN migration tooling scripts.
...
|--------|----------|-------------|
| `--csv` | Yes | CSV file with `hostname`, `propertyName` columns (at minimum) |
| `--directory` | Yes | Directory containing per-property ruletree JSON files (filenames match CSV `templateName` or `propertyName`) |
| `--contract` | Yes | Contract ID (starts with `ctr`) |
| `--group` | Yes | Group ID (starts with `grp`) |
| `--network` | No | `ENHANCED_TLS` or `STANDARD_TLS` (default: `STANDARD_TLS`) |
| `--product` | No | Product ID override (e.g. `prd_Fresca`, `prd_SPM`, `prd_Site_Accel`). If omitted, read from CSV `product` column |
| `--cert-mode` | No | `SBD` (Secure by Default) or `CPS` (CPS Managed). Default: `SBD` |
| `--enrollment-id` | No | CPS enrollment ID. Creates one CPS_MANAGED edge hostname per property. Requires `--cert-mode CPS` |
| `--use-existing-edgehostname` | No | Use existing edge hostnames. Pass an EHN name to apply a single EHN to all properties, or omit the value to read from the CSV `edgeHostname` column |
| `--rule-format` | No | Rule format version (default: `latest`) |
| `--use-cpcode` | No | Existing CP Code (numeric) to reuse for all hostnames |
| `--gtm-domain` | No | GTM domain for hostname replacement in ruletrees |
| `--media-ehn` | No | AMD edge hostname option: `VOD` or `LIVE` (default: `VOD`) |
| `--activate` | No | Activation targets: `staging`, `production` (repeatable) |
| `--email` | No | Notification email(s) for activations (repeatable) |
| `--force` | No | Skip confirmation prompt |
| `--launch/--no-launch` | No | Auto-open Excel output (default: `--launch`) |

### Certificate Modes

**SBD (default):** Edge hostnames are auto-created on property activation using Secure by Default certificates.

**CPS:** For properties requiring CPS-managed certificates. Three sub-modes:

- `--cert-mode CPS --enrollment-id N` -- creates one edge hostname per property using the CPS enrollment
- `--cert-mode CPS --use-existing-edgehostname my-ehn.edgekey.net` -- assigns the provided EHN to all hostnames
- `--cert-mode CPS` (no other flags) -- creates a placeholder edge hostname on the account (`{accountId}-placeholder.edgesuite.net`) so onboarding can proceed before the CPS enrollment is ready

### CSV Input File

Minimum required columns: `hostname`, `propertyName`

Optional columns: `product`, `edgeHostname`, `secureNetwork`, `templateName`, `GroupID`, `orgId`

```
hostname,propertyName,product
www.example.com,example-property,prd_Fresca
api.example.com,example-property,prd_Fresca
blog.example.com,blog-property,prd_Site_Accel
```

### Output

Generates an Excel workbook at `output/{account_name}/{timestamp}conversion-result.xlsx` with:
- **properties** sheet: property IDs, hostnames, edge hostnames, CP codes, error flags
- **activation_status** sheet: activation results (if `--activate` was used)
